...
public class DragSnapper : UIBehaviour, IEndDragHandler, IBeginDragHandler {
    ...
    private int itemCount; // Item count in our scroll rect (2 minimum)

    private ScrollRect scrollRect;
    private int target; // Last target snap position.
    private float value; // Last valid snapped position.
    ...
    private IEnumerator SnapRect() {
        ...
        float duration =
            Mathf.Abs((endNormal - startNormal) / speed); // Time needed to get the target depending on speed.
        ...
        while (timer < 1f) // Animate deceleration.
        {
            ...
            value = Mathf.Lerp(startNormal, endNormal,
                decelerationCurve.Evaluate(timer)); // Our value based on the animation curve.
