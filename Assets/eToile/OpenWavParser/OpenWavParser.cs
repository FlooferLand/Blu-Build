...
public static class OpenWavParser {
    ...
    public static AudioClip ByteArrayToAudioClip(byte[] wavFile, string name = "", bool stream = false) {
        ...
        if (IsWAVFile(wavFile)) {
            ...
            var _bitsPerSample = (Resolution)BitConverter.ToInt16(wavFile, 34);
            // PCM WAV method:
            if (_audioFormat == 1) {
                ...
                var audioClip = AudioClip.Create(name, _sampleCount / _numChannels, _numChannels, _sampleRate, stream);
    ...
    public static AudioClip StereoToMono(AudioClip stereoClip, bool stream = false) {
        ...
        var audioClip = AudioClip.Create(stereoClip.name, _audioBuffer.Length, 1, stereoClip.frequency, stream);
        audioClip.SetData(_audioBuffer, 0);
        return audioClip;
    }
    ...
    public static AudioClip MonoToStereo(AudioClip monoClip, bool stream = false) {
        ...
        var audioClip = AudioClip.Create(monoClip.name, monoClip.samples, 2, monoClip.frequency, stream);
        audioClip.SetData(_audioBuffer, 0);
        return audioClip;
    }
}