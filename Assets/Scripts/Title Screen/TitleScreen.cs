...
public class TitleScreen : MonoBehaviour {
    ...
    private bool barUp;
    private CanvasGroup characterG;
    private CanvasGroup creditsG;
    private Animator downButtonAnimator;
    private CanvasGroup faqG;

    private bool isHalloween;

    private CanvasGroup settingG;
    ...
    private void Start() {
        ...
        if (disableRaytracing) {
            if (PlayerPrefs.GetInt("Settings: SSR") >= 5) PlayerPrefs.SetInt("Settings: SSR", 4);
            if (PlayerPrefs.GetInt("Settings: SSAO") >= 3) PlayerPrefs.SetInt("Settings: SSAO", 1);
        }
    ...
    private void FixedUpdate() {
        if (sceneLoadCache != "") SceneLoadCheck();
        ...
        if (!stopUpdate) {
            ...
            if (settingG.alpha <= 0) settingW.SetActive(false);
            if (faqG.alpha <= 0) faqW.SetActive(false);
            if (creditsG.alpha <= 0) creditsW.SetActive(false);
        ...
        if (sceneLoadCache == "") {
            ...
            if (!sandbox) {
                if (tutorialAskPopup) {
                    ...
                    foreach (var t in bars) t.transition = true;
    ...
    private void SceneLoadCheck() {
        if (fadeWhichWay && fade.color.a >= 1) SceneManager.LoadScene(sceneLoadCache, LoadSceneMode.Single);
    }
