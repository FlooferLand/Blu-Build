...
public class PlayMenuManager : MonoBehaviour {
    ...
    private void LateUpdate() {
        if (playUI.manager.playMovements) {
            ...
            tickTimerRect.anchoredPosition =
                new Vector2(-490 + Mathf.Min(Mathf.Max(nowtimer / (float)(endtexttimer + 1), 0), 1) * 980,
                    tickTimerRect.localPosition.y);
    ...
    public void TextUpdate(bool record) {
        if (!playUI.manager.recordMovements) {
            ...
            foreach (Transform child in transform) {
                var check = child.GetComponent<Button3D>();
