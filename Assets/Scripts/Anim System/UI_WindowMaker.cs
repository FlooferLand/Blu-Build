...
public class UI_WindowMaker : MonoBehaviour {
    ...
    public void MakeThreeWindow(Sprite one, Sprite two, Sprite three, int switchBack, int switchOne, int switchTwo,
        int switchThree, string butOne, string butTwo, string butThree, int fontSize) {
        DisableWindows();
    ...
    public void MakeShowtapeWindow(int add) {
        ...
        if (allShowtapes[currentYear].groups.Length == 0) {
            ...
            for (int i = 1; i < 16; i++) ShowtapeWindow.transform.Find("b" + i).gameObject.SetActive(false);
        }
        else {
            ...
            for (int i = 1; i < 17; i++)
                if (i - 1 < allShowtapes[currentYear].groups.Length) {
                    ...
                    ShowtapeWindow.transform.Find("b" + i).transform.Find("Text").GetComponent<Text>().text =
                        allShowtapes[currentYear].groups[i - 1].showtapeName;
                    ShowtapeWindow.transform.Find("b" + i).transform.Find("Text").GetChild(0).GetComponent<Text>()
                        .text = allShowtapes[currentYear].groups[i - 1].showtapeDate;
                    if (allShowtapes[currentYear].groups[i - 1].segments.Length == 0) {
                        ShowtapeWindow.transform.Find("b" + i).transform.Find("Text").GetComponent<Text>().color =
                            new Color32(188, 119, 0, 255);
                        ShowtapeWindow.transform.Find("b" + i).transform.Find("Text").GetChild(0).GetComponent<Text>()
                            .color = new Color32(198, 176, 0, 255);
                        ShowtapeWindow.transform.Find("b" + i).transform.Find("Length").GetComponent<Text>().color =
                            Color.gray;
                    }
                    else {
                        ShowtapeWindow.transform.Find("b" + i).transform.Find("Text").GetComponent<Text>().color =
                            new Color32(255, 162, 0, 255);
                        ShowtapeWindow.transform.Find("b" + i).transform.Find("Text").GetChild(0).GetComponent<Text>()
                            .color = new Color32(255, 227, 0, 255);
                        ShowtapeWindow.transform.Find("b" + i).transform.Find("Length").GetComponent<Text>().color =
                            Color.white;
                    }
                    ...
                    if (allShowtapes[currentYear].groups[i - 1].segments.Length != 0) {
                        ...
                        for (int e = 0; e < allShowtapes[currentYear].groups[i - 1].segments.Length * 2; e++)
                            if (e % 2 == 0) {
                                adding[e] = Convert.ToInt32(allShowtapes[currentYear].groups[i - 1].segments[j]
                                    .showLength.Split(':')[0]);
                            }
                            else {
                                adding[e] = Convert.ToInt32(allShowtapes[currentYear].groups[i - 1].segments[j]
                        ...
                        if (hours > 0)
                            ShowtapeWindow.transform.Find("b" + i).transform.Find("Length").GetComponent<Text>().text =
    ...
    public void MakeSegmentWindow(int current) {
        DisableWindows();
        if (current == -1)
            currentSegment = Mathf.Min(currentSegment + 1,
                allShowtapes[currentYear].groups[currentGroup].segments.Length - 1);
        else if (current == -2)
        ...
        if (allShowtapes[currentYear].groups[currentGroup].segments.Length == 0) {
            ...
            SegmentWindow.transform.Find("Show Name").GetComponent<Text>().text =
                allShowtapes[currentYear].groups[currentGroup].showtapeName;
            SegmentWindow.transform.Find("Segment Name").GetComponent<Text>().text = allShowtapes[currentYear]
                .groups[currentGroup].segments[currentSegment].segmentNumber;
            SegmentWindow.transform.Find("Segment").GetComponent<Text>().text = allShowtapes[currentYear]
                .groups[currentGroup].segments[currentSegment].showName.Replace("\\n", "\n");
            SegmentWindow.transform.Find("Length").GetComponent<Text>().text = "Length: (" +
                                                                               allShowtapes[currentYear]
                                                                                   .groups[currentGroup]
    ...
    public void MakeRecordIconsWindow() {
        ...
        for (int i = 0; i < 24; i++)
            if (i < recordingGroups.Length) {
                var button = RecordIconsWindow.transform.Find("Button (" + i + ")").gameObject;
    ...
    public void MakeMoveTestWindow(int currentGroup) {
        ...
        for (int i = 0; i < 33; i++)
            if (i < recordingGroups[currentGroup].inputNames.Length) {
                var button = MoveTestWindow.transform.Find("Button " + i).gameObject;
