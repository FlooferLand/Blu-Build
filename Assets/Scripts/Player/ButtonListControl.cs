...
public class ButtonListControl : MonoBehaviour {
    ...
    private void OnDisable() {
        foreach (Transform child in buttonHolder.transform) Destroy(child.gameObject);
    }
    ...
    public void CreateAttributePage(string icon) {
        //Delete previous buttons
        foreach (Transform child in attributeHolder.transform) Destroy(child.gameObject);
        attributeCount = 0;
        ...
        for (int i = 0; i < gg.prizeStrings[e].prizeStrings.Length; i++)
            if (currentItemName == gg.prizeStrings[e].prizeStrings[i].name) {
                ...
                attImage.sprite = Resources.Load<Sprite>("Merch/Icons/" + currentItemName);
                ...
                for (int j = 0; j < gg.prizeStrings[e].prizeStrings[i].attributes.Length; j++)
                    AddGenericAttribute(gg.prizeStrings[e].prizeStrings[i],
                        gg.prizeStrings[e].prizeStrings[i].attributes[j]);
            }
    }
                ...
                if (gg.prizeStrings[e].prizeStrings[i].skins.Length > 0) {
                    if (gg.prizeStrings[e].prizeStrings[i].skins[0] == "Building Block") {
                        ...
                        if (dropdown.value == 0)
                            ...
                            attImage.sprite = Resources.Load<Sprite>("Merch/Icons/" + currentItemName + dropdown.value);
                    }
                }
    ...
    private void AddSkinsAttribute(PrizeStrings ps) {
        var att = Instantiate(atttempSkin);
        att.SetActive(true);
        att.name = "Skins";
        att.transform.parent = attributeHolder.transform;
        att.GetComponent<RectTransform>().anchoredPosition =
            new Vector2(342.7f + 310 * attributeCount, 790 - 90 * (attributeCount / 4));
        ...
        if (ps.skins[0] == "Building Block") {
            ...
            for (int i = 0; i < gg.buildMaterials.Length; i++)
                att.transform.GetComponentInChildren<Dropdown>().options
                    .Add(new Dropdown.OptionData(gg.buildMaterials[i].name));
        }
        else {
    ...
    private void AddGenericAttribute(PrizeStrings ps, string theCase) {
        GameObject att = null;
        switch (theCase) {
            case "Scale":
        ...
        att.transform.parent = attributeHolder.transform;
        att.GetComponent<RectTransform>().anchoredPosition =
            new Vector2(334.0f + 305 * attributeCount, 790 - 85 * (attributeCount / 4));
        att.GetComponent<RectTransform>().localScale = new Vector3(1, 1, 1);
        attributeCount++;
    }
    ...
    public void CreatePrize() {
        ...
        float length = 7;
        while (true) {
            if (Physics.SphereCast(Camera.main.transform.position, 2, Camera.main.transform.forward, out hit, length,
                    uiLayerMask)) {
                length -= 0.01f;
            }
            else {
        ...
        var ez = GameObject.Find("GlobalAudio").GetComponent<AudioSource>();
        ez.clip = (AudioClip)Resources.Load("Create");
        ez.pitch = 1.0f;
        ez.Play();
        var finalPoint = Camera.main.transform.position + Camera.main.transform.forward * length;
        var prize = Instantiate(Resources.Load("Merch/Prefabs/" + currentItemName) as GameObject, finalPoint,
            Quaternion.identity);
        ...
        for (int i = 0; i < gg.prizeStrings[e].prizeStrings.Length; i++)
            if (currentItemName == gg.prizeStrings[e].prizeStrings[i].name) {
                ...
                if (gg.prizeStrings[e].prizeStrings[i].skins.Length > 0) {
                    string skin;
                    if (gg.prizeStrings[e].prizeStrings[i].skins[0] == "Building Block")
                        skin = attributeHolder.transform.Find("Skins").Find("Dropdown").GetComponent<Dropdown>()
                            .options[
                                attributeHolder.transform.Find("Skins").Find("Dropdown").GetComponent<Dropdown>().value]
                            .text;
                    else
                        skin = gg.prizeStrings[e].prizeStrings[i]
                            .skins[
                                attributeHolder.transform.Find("Skins").Find("Dropdown").GetComponent<Dropdown>()
                                    .value];
                ...
                for (int j = 0; j < gg.prizeStrings[e].prizeStrings[i].attributes.Length; j++)
                    switch (gg.prizeStrings[e].prizeStrings[i].attributes[j]) {
                        case "Scale":
                            float scale = attributeHolder.transform
                                .Find(gg.prizeStrings[e].prizeStrings[i].attributes[j]).Find("Slider")
                            ...
                            break;
                        case "Complex Scale":
                            var sc = attributeHolder.transform.Find(gg.prizeStrings[e].prizeStrings[i].attributes[j])
                                .gameObject;
                            var cscale = new Vector3(
                                float.Parse(sc.transform.Find("X").GetComponent<InputField>().text),
                                float.Parse(sc.transform.Find("Y").GetComponent<InputField>().text),
                                float.Parse(sc.transform.Find("Z").GetComponent<InputField>().text));
                            ...
                            break;
                        case "Light Temperature":
                            var lighttemp = attributeHolder.transform
                                .Find(gg.prizeStrings[e].prizeStrings[i].attributes[j]).gameObject;
                            ...
                            break;
                        case "Light Intensity":
                            var lightinten = attributeHolder.transform
                                .Find(gg.prizeStrings[e].prizeStrings[i].attributes[j]).gameObject;
                            ...
                            foreach (Transform child in prize.transform)
                                child.SendMessage("ATTLightIntensity", intensity,
                                    SendMessageOptions.DontRequireReceiver);
                            break;
                        case "Light Spot Size":
                            var spotsize = attributeHolder.transform
                                .Find(gg.prizeStrings[e].prizeStrings[i].attributes[j]).gameObject;
                            ...
                            var both = new Vector2(outer, inner);
                            ...
                            break;
                        case "Light Color":
                            var color = attributeHolder.transform.Find(gg.prizeStrings[e].prizeStrings[i].attributes[j])
                                .gameObject;
                            var colorreal = color.transform.Find("Color").GetComponent<Image>().color;
                            ...
                            break;
                        case "Material Color":
                            var cc = attributeHolder.transform.Find(gg.prizeStrings[e].prizeStrings[i].attributes[j])
                                .gameObject;
                            var cr = cc.transform.Find("Color").GetComponent<Image>().color;
