...
public class SecurityCams : MonoBehaviour {
    ...
    private void Update() {
        if (Vector3.Distance(player.transform.position, transform.position) < 5) {
            if (!on) cam.gameObject.SetActive(true);
            on = true;
        }
        else {
            if (on) cam.gameObject.SetActive(false);
            on = false;
        }
    ...
    public void UpdateCams() {
        var poses = new Vector2[cams.Length];
        for (int i = 0; i < cams.Length; i++) poses[i] = new Vector2(cams[i].position.x, cams[i].position.z);
        float minX = 0;
        float minY = 0;
        float maxX = 0;
        float maxY = 0;
        for (int i = 0; i < poses.Length; i++) {
            ...
            if (poses[i].x < minX) minX = poses[i].x;
            if (poses[i].y < minY) minY = poses[i].y;
            if (poses[i].x > maxX) maxX = poses[i].x;
            if (poses[i].y > maxY) maxY = poses[i].y;
        }
        ...
        for (int i = 0; i < buttons.Length; i++) buttons[i].GetComponent<RectTransform>().localPosition = poses[i];
    }
