...
public class VersionManager : Editor {
    [HideInInspector] public bool version;

    [HideInInspector] public bool VR;

    public override void OnInspectorGUI() {
        DrawDefaultInspector();
        var exam = (SimulatorManager)target;
        ///////////////////////////////////////////////////////////////////////////////////////////
        GUILayout.Space(20);
        if (VR)
        ...
        if (version)
            ...
            GUI.backgroundColor = Color.grey;
        if (GUILayout.Button("Set Faz-Anim")) SetFazAnim();
        ...
        if (version)
            ...
            GUI.backgroundColor = Color.green;
        if (GUILayout.Button("Set Rock-afire Replay")) SetSimulator();
        if (EditorGUI.EndChangeCheck() && !version) {
            if (VR)
    ...
    public void SetFazAnim() {
        ...
        for (int i = 0; i < 8; i++) tex[i] = exam.fazTexture as Texture2D;
    ...
    public void SetSimulator() {
        ...
        var tex = new Texture2D[8];
        for (int i = 0; i < 8; i++) tex[i] = exam.rockTexture as Texture2D;
        PlayerSettings.SetIcons(NamedBuildTarget.Standalone, tex, IconKind.Any);
    }
