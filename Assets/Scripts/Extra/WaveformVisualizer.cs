...
public class WaveformVisualizer : MonoBehaviour {
    ...
    private void Start() {
        var tex = new Texture2D(Screen.width, height, TextureFormat.RGBA32, false);
    ...
    public void PaintWaveformSpectrum(float viewzoomMin, float viewZoomMax, float audioLengthMax) {
        if (audioClip != null) {
            ...
            var tex = new Texture2D(Screen.width / 2, height, TextureFormat.RGBA32, false);
            ...
            for (int i = offset; i + packSize < offset + length; i += packSize) {
                ...
                for (int e = 0; e < packSize; e++) {
                    ...
                    e += Mathf.FloorToInt(packSize / (float)reducedPackSize - 1);
                }

                waveform[s] = samp / reducedPackSize;
                s++;
            }
