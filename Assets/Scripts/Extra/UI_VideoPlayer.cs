...
public class UI_VideoPlayer : MonoBehaviour {
    ...
    private Text startTT;
    private VideoPlayer TVVid;
    ...
    private void Update() {
        if (TVVid.isPlaying) {
            ...
            startTT.text = Mathf.Floor((int)TVVid.time / 60f).ToString("00") + ":" +
                           ((int)TVVid.time % 60).ToString("00");
        }
    }
    ...
    public void UpdatePage(int add) {
        ...
        windowYear.transform.Find("yearText").GetComponent<Text>().text = 1970 + currentPage * 10 + "'s";

        for (int i = 0; i < 10; i++) {
            Color buttoncolor;
            var vF = videoFolders[currentPage * 10 + i];
            float totalVids = vF.Diagnostic.Length + vF.Promotional.Length + vF.Installation.Length +
                              vF.Training.Length + vF.Commercial.Length + vF.News.Length + vF.Amatuer.Length +
                              vF.Other.Length;
            ...
            windowYear.transform.Find(i.ToString()).transform.Find("Text").GetComponent<Text>().text =
                (1970 + currentPage * 10 + i).ToString();
            windowYear.transform.Find(i.ToString()).transform.Find("Text (1)").GetComponent<Text>().text =
                totalVids.ToString();
        }
    }
    ...
    public void GoToYear(int add) {
        ...
        var vF = videoFolders[currentPage * 10 + add];
        float totalVids = vF.Diagnostic.Length + vF.Promotional.Length + vF.Installation.Length + vF.Training.Length +
                          vF.Commercial.Length + vF.News.Length + vF.Amatuer.Length + vF.Other.Length;
        ...
        if (totalVids != 0) {
            ...
            windowVideo.SetActive(false);
            windowPlayer.SetActive(false);
            windowType.transform.Find("yearText").GetComponent<Text>().text =
                (1970 + currentPage * 10 + add).ToString();
            ...
            for (int i = 0; i < 8; i++) {
                ...
                windowType.transform.Find(i.ToString()).transform.Find("Text (1)").GetComponent<Text>().color =
                    Color.black;
                windowType.transform.Find(i.ToString()).transform.Find("Text (1)").GetComponent<Text>().text =
                    vidCount.ToString();
            }
        }
    }
    ...
    public void MakeVideo(int type) {
        ...
        if (selection.Length != 0) {
            ...
            windowVideo.transform.Find("description").GetComponent<Text>().text =
                selection[currentVideo].videoDescription;
    ...
    public void PlayVideo(int input) {
        ...
        PauseVideo(-64);
        LoopVideo(-64);
        endTT.text = Mathf.Floor((int)TVVid.clip.length / 60f).ToString("00") + ":" +
                     ((int)TVVid.clip.length % 60).ToString("00");
    }
