...
namespace AYellowpaper.SerializedCollections.Editor.Search {
    public class NumericMatcher : Matcher {
        ...
        public override bool IsMatch(SerializedProperty property) {
            if (property.propertyType == SerializedPropertyType.Float) return IsFloatMatch(property.floatValue);

            if (property.propertyType == SerializedPropertyType.Integer) return IsIntMatch(property.intValue);
            ...
            if (property.propertyType == SerializedPropertyType.Vector4) return IsVector4Match(property.vector4Value);
            return false;
        }

        private bool IsFloatMatch(float val) {
            string str = val.ToString(CultureInfo.InvariantCulture);
            return str.Contains(SearchString, StringComparison.OrdinalIgnoreCase);
        }
